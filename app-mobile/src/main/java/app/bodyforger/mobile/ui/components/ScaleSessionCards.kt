...
internal fun Pairing(state: ScaleUiState) {
    Card {
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            Text(stringResource(R.string.scale_pairing_in_progress), color = TextPrimary, fontSize = 15.sp, fontWeight = FontWeight.SemiBold)
        }
        ...
        state.pairingInstructions.forEach { instruction ->
            Text(stringResource(instructionLabel(instruction)), color = TextPrimary, fontSize = 14.sp, fontWeight = FontWeight.Medium)
        }
        Text(
            stringResource(R.string.scale_calibration_note),
            color = TextSecondary,
            fontSize = 12.sp,
...
internal fun Failure(failure: SessionFailure) {
    Card {
        Text(stringResource(R.string.scale_weigh_in_interrupted), color = TextPrimary, fontSize = 14.sp, fontWeight = FontWeight.SemiBold)
        Text(stringResource(failureLabel(failure)), color = TextSecondary, fontSize = 12.sp, modifier = Modifier.padding(top = 4.dp))
    }
}
