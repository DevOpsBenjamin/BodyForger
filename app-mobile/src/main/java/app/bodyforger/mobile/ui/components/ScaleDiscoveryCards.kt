...
internal fun NotAssociated(onStartScan: () -> Unit) {
    Card {
        ...
        ) { Text(stringResource(R.string.scale_search), fontWeight = FontWeight.Bold) }
    }
}
...
internal fun Scanning(
    ...
    Card {
        Row(verticalAlignment = Alignment.CenterVertically) {
            ...
            Text(stringResource(R.string.scale_searching), color = TextPrimary, fontSize = 14.sp)
        }
        if (discovered.isEmpty()) {
            Text(
                stringResource(R.string.scale_none_found),
                color = TextSecondary,
                fontSize = 12.sp,
        ...
        if (others.isNotEmpty()) {
            Text(
                stringResource(R.string.scale_other_devices),
                color = TextSecondary,
                fontSize = 11.sp,
        ...
        OutlinedButton(onClick = onStopScan, modifier = Modifier.fillMaxWidth().padding(top = 12.dp)) {
            Text(stringResource(R.string.scale_stop), color = TextSecondary)
        }
    }
}
