...
class TrainingStatsTest {
    ...
    fun `records are listed heaviest first, one per exercise`() {
        val sessions = listOf(
            session(sets = listOf(set("Squat", 140.0, 1), set("Bench Press", 100.0, 1), set("Squat", 120.0, 1)))
        )
        val records = TrainingStats.personalRecords(sessions)
        assertEquals(listOf("Squat", "Bench Press"), records.map { it.exerciseName })
    }
    ...
    fun `the active days are counted back from today, oldest first`() {
        ...
        // The window spans index 0 (six days ago) through 6 (today).
