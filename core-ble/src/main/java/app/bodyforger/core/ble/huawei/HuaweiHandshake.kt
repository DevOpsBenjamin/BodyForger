...
class HuaweiHandshake(
    ...
    suspend fun negotiate(macAddress: String): ByteArray? {
        ...
        if (scaleNonce == null) {
            Log.w(TAG, "scale nonce missing or too short (${answer?.size ?: 0} o)")
            return null
        }
        ...
        if (scaleToken == null) {
            Log.w(TAG, "the scale did not answer with its token")
            return null
        }
        ...
        if (exchange(HuaweiCharacteristic.SESSION_KEY) {
            ...
            Log.w(TAG, "session key not acknowledged")
            return null
        }
        Log.d(TAG, "negotiation succeeded")
    ...
    private suspend fun exchange(
        ...
        if (!send()) {
            Log.w(TAG, "write refused on $characteristic")
            response.cancel()
