...
class HuaweiPairingSession(
    ...
    fun run(
        ...
        advance() // Wake-up and targeted scan
        ...
        advance() // Engraving the HUID — irreversible from here on
        ...
        advance() // Waiting on the tare: the athlete must step on
        ...
        advance() // User profile, carrying the tare actually read
        writeSealed(
            sessionKey,
        ...
        advance() // Arming the BIA stream — the athlete is still on the scale
        ...
        advance() // Confirmation reading, then acknowledgement
    ...
    private suspend fun awaitTare(
        ...
        if (status != STATUS_OK) {
            Log.w(TAG, "engraving refused by the scale (statut $status)")
            return null
        }
