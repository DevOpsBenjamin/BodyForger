...
class AndroidGattTransport(
    ...
    override suspend fun connect(): Boolean {
        repeat(CONNECTION_ATTEMPTS) { attempt ->
            ...
            Log.w(TAG, "connection attempt ${attempt + 1}/$CONNECTION_ATTEMPTS failed")
            closeInternal()
    ...
    private suspend fun attemptConnect(): Boolean = gattLock.withLock {
        ...
        if (withTimeoutOrNull(CONNECTION_TIMEOUT_MS) { connected.await() } != true) {
            Log.w(TAG, "connection refused or timed out")
        ...
        Log.d(TAG, if (ready) "services discovered" else "service discovery failed")
    ...
    private suspend fun setNotification(
        ...
        if (!started) {
            Log.w(TAG, "descriptor write did not start: $characteristic")
            return@withLock false
        }
        ...
        Log.d(TAG, "subscription $characteristic: ${if (ok) "ok" else "FAILED (descriptor not acknowledged)"}")
        ok
    }
    ...
    private suspend fun writeFrame(
        ...
        val target = resolve(characteristic)
        if (target == null) {
            Log.w(TAG, "characteristic not found on write : $characteristic")
            return@withLock false
        }
        val connected = gatt ?: return@withLock false
        ...
        if (!started) {
            Log.w(TAG, "write did not start: $characteristic")
            return@withLock false
        }

        val ok = withTimeoutOrNull(operationTimeoutMs) { acknowledged.await() } == true
        Log.d(TAG, "write $characteristic (${frame.size} B): ${if (ok) "ok" else "ÉCHEC"}")
        ok
    }
