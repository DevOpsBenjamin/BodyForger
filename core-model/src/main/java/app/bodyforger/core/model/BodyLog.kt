...
data class BodyCompositionReport(
    ...
    /** Fat over the five segments, parallel to [segmentalMuscle]. `null` when unavailable. */
    val segmentalFat: SegmentalFatMass? = null,

    /** Visceral fat level (integer scale). `null` when the engine does not produce it. */
    val visceralFatLevel: Int? = null,

    /** Basal metabolic rate, kcal/day. `null` when unavailable. */
    val basalMetabolismKcal: Double? = null,

    /** Overall body score (0-100). `null` when unavailable. */
    val bodyScore: Double? = null,

    /** Metabolic body age in years. `null` when unavailable. */
    val metabolicBodyAge: Int? = null,
) {
    /**
